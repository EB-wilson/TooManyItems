...
class ItemTurretParser: TurretParser<ItemTurret>() {
  override fun isTarget(content: Block): Boolean {
    return content is ItemTurret
  }

  override fun parse(content: ItemTurret): Seq<Recipe> {
    val res = Seq<Recipe>()

    content.ammoTypes.each { item, ammoType ->
      ...
      registerCons(recipe, *content.consumers.filterNot {
        it == content.coolant
        || (it is ConsumeItemFilter
        && it::class.java.isAnonymousClass
        && it::class.java.enclosingMethod.declaringClass == ItemTurret::class.java)
      }.toTypedArray())
